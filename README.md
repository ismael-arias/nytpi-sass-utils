...
A collection of utilities (including Size Classes) to make your stylesheet development more pleasant and awesome.

**Note:** these utilities are in a _alpha_ stage of development. Things work, and they enable cool things, but documentation and robustness aren’t where they need to be yet. Please let [@kohlmannj][1] know what bugs or design issues you find!

## Installation
NYTPI Sass Utilities is an npm package, which you can easily install and update using `npm`. In Terminal, in the root level of your Node-based project, run this command:

`$ npm install https://github.com/nytpi/nytpi-sass-utils --save-dev`

### Installing Susy for Size Classes

Note that the `size-class` mixins require that you install [the Susy grid layout framework][2] as a direct devDependency in your project:

`$ npm install susy --save-dev`

(In the future, approaches to resolving Sass libraries by name may eliminate this step. [Eyeglass][3] is one such approach.)
...
- There's a bug in the code used to merge multiple media queries into one, which may result in outputting a CSS media query with a redundant `min-width` that's superseded by a wider `min-width` (for example: `@media (min-width: 540px) and (min-width: 1605px)`)
- Using the `font-size` mixin with `vw` units within [a Susy nested context][5] results in the wrong `vw` font-size being calculated. (More below!)
...
Susy's powerful grid system supports [nested contexts][8], which are critical for correctly sizing elements with Susy within child elements. For example, imagine we want to set up a container like this:
